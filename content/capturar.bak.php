...
<script type="text/javascript" src="include/Jquery-Photo-Tag-master/libraries/jquery-ui-1.8.17.custom.min.js"></script>
...
<?PHP
    ...
    if($_SESSION[TIPO_USR_SESSION]!=1 && isset($_GET['id'])) {
    ...
    //$objHTML->startForm('capturaPaciente', '?mod=cap&id='.$_GET['id'], 'POST');
	$objHTML->startForm('capturaPaciente', 'docs/creaExcel.php', 'POST', array("target='_blank'"));
		...
		$objHTML->startFieldset();
			...
			$objSelects->selectMunicipio('muniNac', $paciente->idCatEstadoNacimiento ? $paciente->idCatEstadoNacimiento : $_SESSION[EDO_USR_SESSION], NULL, $paciente->idCatMunicipioNacimiento, array('class'=>'validate[required]'));
		...
		$objHTML->startFieldset('', array('id'=>'fs_manejo_caso'));
            ...
            $objSelects->selectEstado('edoReferido', $paciente->idCatEstadoReferido);
	...
	if (!empty($_GET['id'])){
		...
		$objHTML->endForm('actualizar', 'Actualizar', 'limpiar', 'Limpiar');
	}
	else {
		//$objHTML->inputHidden('guardar', 1);
		$objHTML->endForm('guardar', 'Guardar', 'limpiar', 'Limpiar');
	}
?>