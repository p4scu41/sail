	...
	public function buscar() {
		...
		if (is_string($consulta)) {
			...
			while ($registro = devuelveRowAssoc($consulta)) {
                ...
                // Consulta el control de calidad del muestra
                $queryCalidad = 'SELECT [calidadLectura] FROM [controlCalidad] WHERE ';
                
                if($registro['tipo'] == 'histo') {
                    $queryCalidad .= ' [idEstudioHis] = '.$registro['idEstudio'];
                } else { // bacilos
                    $queryCalidad .= ' [idEstudioBac] = '.$registro['idEstudio'];
                }
                
                $consultaCalidad = ejecutaQueryClases($queryCalidad);
                $resultadoCalidad = devuelveRowAssoc($consultaCalidad);
				...
				$estudio->calidad = $resultadoCalidad['calidadLectura'];
