...
class IndicadorExamenContactos {
	...
	public $nombre = "Ex&aacute;men de Contactos";
	// VALORES FIJOS
	...
	// BD ******************************************************************************************
	static $idEstadoPacienteAplicableATotalCasos = "1, 2, 3, 5, 6, 9";	 // TABLA catEstadoPaciente
	...
	public function calcular() {
		...
		if (is_null($this->idCatEstado) || is_null($this->idCatJurisdiccion) || is_null($this->fechaInicio) || is_null($this->fechaFin)) {
			...
			$sql = "SELECT COUNT( DISTINCT cc.idContacto) AS numeroContactosExaminados " .
				"FROM diagnostico d, contactos c, pacientes p, catUnidad u, controlContacto cc, control co " .
				...
				"AND cc.fecha BETWEEN '" . formatFechaObj($this->fechaInicio, 'Y-m-d') . "' AND '" . formatFechaObj($this->fechaFin, 'Y-m-d') . "' " .
			...
			if ($this->idCatJurisdiccion != 0)
				$sql = "SELECT COUNT( DISTINCT cc.idContacto) AS numeroContactosExaminados " .
					"FROM diagnostico d, contactos c, pacientes p, catUnidad u, controlContacto cc, catMunicipio m, control co " .
					"WHERE c.idDiagnostico = d.idDiagnostico " .
					"AND cc.idContacto = c.idContacto " .
					"AND p.idPaciente = d.idPaciente " .
	...
	function imprimir() {
		...
		echo '<DIV CLASS="datagrid"><TABLE><THEAD><TR><TH COLSPAN="5">' . $estado . "<BR>" . $jurisdiccion . '</TH></TR>' .
			'<TR><TH>Indicador</TH><TH>Est�ndar</TH><TH>Resultado</TH><TH>Ponderacion</TH><TH>�ndice</TH></TR></THEAD>' .
			'<TR><TD>' . $this->nombre .
			'</TD><TD>' . $this->estandar . "%" .
